...
- Added `scripts/uninstall.sh` (stops Flipside, removes the old `.app`, resets the Accessibility TCC grant for `com.flipside.app`), wired into `make_app_bundle.sh` so every rebuild starts clean — every ad-hoc re-sign was leaving stale, ambiguous "Flipside" entries in System Settings > Privacy & Security > Accessibility.
